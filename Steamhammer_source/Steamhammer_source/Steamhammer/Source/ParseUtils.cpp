...
void ParseUtils::ParseConfigFile(const std::string & filename)
{
	...
	// NOTE The bases list attempts to exclude observer slots, but sometimes fails to.
	//      Unrecognized observer slots will inflate the result.
    const int mapSize = the.bases.getStarting().size();
    ...
    if (doc.HasMember("Debug") && doc["Debug"].IsObject())
    {
        ...
        JSONTools::ReadBool("DrawTasks", debug, Config::Debug::DrawTasks); 
    }
    ...
    if (doc.HasMember("Skills") && doc["Skills"].IsObject())
    {
        ...
        JSONTools::ReadBool("UseSunkenRangeBug", skills, Config::Skills::UseSunkenRangeBug);
        JSONTools::ReadBool("StaticDefense", skills, Config::Skills::StaticDefense);
        ...
        if (!openingStrategyDecided && OpponentModel::Instance().getRecommendedOpening() != "")
        {
            ...
            else if (opening.substr(0, 7) == "Counter")
            {
                    ...
                    strategy["CounterStrategies"].IsObject())
                {
                    ...
                    std::string recommendationVersus = recommendation + " v" + RaceChar(the.enemyRace());
...
void ParseUtils::ParseTextCommand(const std::string & commandString)
{
    ...
    if (command == "/set")
    {
        ...
        else if (variableName == "drawtasks") { Config::Debug::DrawTasks = GetBoolFromString(val); }
...
int ParseUtils::GetIntByRace(const char * name, const rapidjson::Value & item)
{
    if (item.HasMember(name))
    {
        ...
        {
            const std::string raceStr(the.selfRace().getName());
